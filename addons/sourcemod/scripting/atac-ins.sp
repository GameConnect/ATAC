...
public void OnPluginStart()
{
    ...
    g_hPointCaptureKarma = CreateConVar("atac_pointcapture_karma", "2",  "ATAC Point Capture Karma");
 ...
 * User Messages
 */
public Action UserMsg_ObjMsg(UserMsg msg_id, Handle bf, const int[] players, int playersNum, bool reliable, bool init)
{
    if (!ATAC_GetSetting(AtacSetting_Enabled)) {
        return;
    }
    ...
    for (int i = 1, iTeam = BfReadByte(bf); i <= MaxClients; i++) {
        if (!IsClientInGame(i) || GetClientTeam(i) != iTeam) {
            continue;
        }
