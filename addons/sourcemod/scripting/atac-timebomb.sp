...
new g_iTimeBombTime[MAXPLAYERS + 1];
...
{
	// Create convars
	g_hTicks = CreateConVar("atac_timebomb_ticks", "10", "ATAC TimeBomb Ticks", FCVAR_PLUGIN);
 ...
 * ATAC Punishments
 */
public AtacPunishment_TimeBomb(victim, attacker)
{
	...
	g_iTimeBombTime[attacker] = GetConVarInt(g_hTicks);
...
public Action:Timer_TimeBomb(Handle:timer, any:client)
{
	...
	if(--g_iTimeBombTime[client])
	{
		...
		{
			iColor = RoundToFloor(g_iTimeBombTime[client] * (255.0 / GetConVarInt(g_hTicks)));
